...
class TestSubfixIntegration(unittest.TestCase):
	...
	def test_pathExistsNoFlags(self):
		...
		with self.tempTestDir(['', 'testing_set.zip'], 'Integration - Homework 0, No flags', answer) as result:
			self.assertTrue(result)

	def test_pathExistsMove(self):
		answer = self.pathTestSetup()
		with self.tempTestDir(['', 'testing_set.zip', '-m'], 'Integration - Homework 0, -move', answer) as result:
			self.assertTrue(result)

	def test_pathExistsPath(self):
		answer = self.pathTestSetup(os.path.join('NewFolder','Homework 0'))
		with self.tempTestDir(['', 'testing_set.zip', '-pNewFolder'], 'Integration - Homework 0, -path', answer) as result:
			self.assertTrue(result)

	def test_pathExistsPathMove(self):
		answer = self.pathTestSetup('NewFolder')
		with self.tempTestDir(['', 'testing_set.zip', '-pNewFolder', '-m'], 'Integration - Homework 0, -path -move', answer) as result:
			self.assertTrue(result)
